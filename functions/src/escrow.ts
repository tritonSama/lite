...
export const releaseEscrow = onCall({ region: 'us-central1', secrets: [STRIPE_SECRET_KEY] }, async (req) => {
  const { taskId } = req.data as { taskId: string };
  const uid = req.auth?.uid;
  if (!uid) throw new HttpsError('unauthenticated', 'Must be signed in');

  const stripe = new Stripe(STRIPE_SECRET_KEY.value(), {
    apiVersion: '2023-10-16' as any, // Use type assertion to avoid TypeScript error on older/newer SDK types
  });
  ...
  await db.runTransaction(async (t) => {
    ...
    if (!task.selectedProviderId) {
       throw new HttpsError(
        'failed-precondition',
        'Task does not have a selected provider',
      );
    }
    ...
    if (!providerStripeId) {
       throw new HttpsError(
        'failed-precondition',
        'Provider does not have a linked Stripe account',
      );
    }
  ...
  if (transferAmountInSmallestUnit > 0) {
    try {
      await stripe.transfers.create({
       ...
       console.error('Stripe transfer failed:', error);
       throw new HttpsError('internal', `Stripe transfer failed: ${error.message || 'Unknown error'}`);
    }
  }
  ...
  await db.runTransaction(async (t) => {
    ...
    const task = taskSnap.data()!;
    if (task.status !== 'submittedForVerification') return;
