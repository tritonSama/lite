...
@JsonKey(includeFromJson: false, includeToJson: false)
@override
int get hashCode => Object.hashAll([runtimeType,id,creatorId,title,description,const DeepCollectionEquality().hash(photoUrls),location,locationLabel,geohash,desiredCompletionDate,budgetAmount,currencyCode,const DeepCollectionEquality().hash(requiredSkills),const DeepCollectionEquality().hash(requiredEquipment),const DeepCollectionEquality().hash(requiredCredentials),workerCount,specialRequirements,status,selectedProviderId,bidCount,category,createdAt,updatedAt]);

@override
String toString() {
  return 'Task(id: $id, creatorId: $creatorId, title: $title, description: $description, photoUrls: $photoUrls, location: $location, locationLabel: $locationLabel, geohash: $geohash, desiredCompletionDate: $desiredCompletionDate, budgetAmount: $budgetAmount, currencyCode: $currencyCode, requiredSkills: $requiredSkills, requiredEquipment: $requiredEquipment, requiredCredentials: $requiredCredentials, workerCount: $workerCount, specialRequirements: $specialRequirements, status: $status, selectedProviderId: $selectedProviderId, bidCount: $bidCount, category: $category, createdAt: $createdAt, updatedAt: $updatedAt)';
}
...
switch (_that) {
case _Task() when $default != null:
return $default(_that.id,_that.creatorId,_that.title,_that.description,_that.photoUrls,_that.location,_that.locationLabel,_that.geohash,_that.desiredCompletionDate,_that.budgetAmount,_that.currencyCode,_that.requiredSkills,_that.requiredEquipment,_that.requiredCredentials,_that.workerCount,_that.specialRequirements,_that.status,_that.selectedProviderId,_that.bidCount,_that.category,_that.createdAt,_that.updatedAt);case _:
  return orElse();
...
class _Task implements Task {
  const _Task({required this.id, required this.creatorId, required this.title, required this.description, final  List<String> photoUrls = const [], @GeoPointConverter() required this.location, required this.locationLabel, required this.geohash, @TimestampConverter() required this.desiredCompletionDate, required this.budgetAmount, this.currencyCode = 'USD', final  List<String> requiredSkills = const [], final  List<String> requiredEquipment = const [], final  List<RequiredCredential> requiredCredentials = const [], this.workerCount = 1, this.specialRequirements, this.status = TaskStatus.draft, this.selectedProviderId, this.bidCount = 0, this.category = TaskCategory.cleanup, @TimestampConverter() required this.createdAt, @TimestampConverter() required this.updatedAt}): _photoUrls = photoUrls,_requiredSkills = requiredSkills,_requiredEquipment = requiredEquipment,_requiredCredentials = requiredCredentials;
...
@JsonKey(includeFromJson: false, includeToJson: false)
@override
int get hashCode => Object.hashAll([runtimeType,id,creatorId,title,description,const DeepCollectionEquality().hash(_photoUrls),location,locationLabel,geohash,desiredCompletionDate,budgetAmount,currencyCode,const DeepCollectionEquality().hash(_requiredSkills),const DeepCollectionEquality().hash(_requiredEquipment),const DeepCollectionEquality().hash(_requiredCredentials),workerCount,specialRequirements,status,selectedProviderId,bidCount,category,createdAt,updatedAt]);
